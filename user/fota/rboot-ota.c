...
// Changed HTTP header parser to adapt to multiple
// servers
//////////////////////////////////////////////////
...
{
	//Variable used to parse the HTML header
	static uint8_t _header_line = 0;

	char *ptrLen = pusrdata;

	// disarm the timer
	os_timer_disarm(&ota_timer);

	//HTTP header parser
	while ((os_strstr(ptrLen, "\r\n") != 0) && (_header_line != 200))
	{
		os_printf("header line\r\n");
		++_header_line;
		if (_header_line == 1)
		{
			//validate HTTP 200 response from the server
			if (os_strstr(ptrLen, "200") != 0)
			{
				os_printf("HTTP 200 received\r\n");
			}
			else
			{
				os_printf("HTTP response error\r\n");
				rboot_ota_deinit();
				_header_line = 0;
				return;
			}
		...
		else if ((((char*)os_strstr(ptrLen, "Content-Length: ") != 0) && (upgrade->content_len == 0)))
		{
			os_printf("data length :");
			os_printf((char*)os_strstr(ptrLen, "Content-Length: ") + 16);
			//Populate the content length on to the upgrade struct
			upgrade->content_len = atoi(((char*)os_strstr(ptrLen, "Content-Length: ")) + 16);
		}
		else if ((char*)os_strstr(ptrLen, "\r\n\r\n") != 0)
		{
			os_printf("End of header\r\n");
			//calculate length of data that came with the header chunk(4 characters at end of header)
			length -= ( ((char*)os_strstr(pusrdata, "\r\n\r\n") + 4)  - pusrdata);
			pusrdata = (char*)os_strstr(pusrdata, "\r\n\r\n") + 4;
			//200 means header valid
			_header_line = 200;
			break;

		}
		else if (_header_line > 10)
		{
			os_printf("More than expected lines in header\r\n");
			_header_line = 0;
			rboot_ota_deinit();
			return;
		}
		ptrLen = ((char*)os_strstr(ptrLen, "\r\n") + 2);
	}


	if ((length > 0) && (_header_line == 200))
	{
		...
		//os_printf("new packet write finished\r\n");
	}

	// check if we are finished
	if ((upgrade->total_len == upgrade->content_len) && (_header_line == 200))
	{
		os_printf("finished transfer\r\n");
...
bool ICACHE_FLASH_ATTR rboot_ota_start(ota_callback callback) {
	...
	err_t result;
	os_printf("Enter upgrade function\r\n");
	// check not already updating
	if (system_upgrade_flag_check() == UPGRADE_FLAG_START) {
		os_printf("Already upgrading\r\n");
		return false;
	}
